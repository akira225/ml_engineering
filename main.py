...
def classify(
    item, matrix_of_objects, matrix_of_labels, dist_func, ker_func, window_size
):
    sum_yw = np.zeros(len(matrix_of_labels[0]))
    sum_w = 1e-10
    for i in range(len(matrix_of_objects)):
        sum_yw += matrix_of_labels[i] * ker_func(
            dist_func(matrix_of_objects[i], item) / window_size
        )
...
def validation(
    ...
    for i in range(len(matrix_of_objects)):
        ...
        window_size = window_size_calculation(
            item, cur_step_matrix_of_objects, dist_func, window_size_parameter
        )
        predicted_label = classify(
            item,
            cur_step_matrix_of_objects,
            cur_step_matrix_of_labels,
            dist_func,
            ker_func,
            window_size,
        )
        ...
        for ker in kernel:
            for fixed in fixed_window_size:
                if fixed:
                    ...
                    distances = np.arange(
                        max_d * 1.2 / math.sqrt(len(x)),
                        max_d,
                        max_d / math.sqrt(len(x)),
                    )
                    for distance in distances:
                        print(dist, ker, fixed_window_size_calc, distance)
                        models.append(
                            validation(
                                x, y, dist, ker, fixed_window_size_calc, distance
                            )
                        )
                else:
                    i += 1
                    ...
                    for distance in distances:
                        ...
                        models.append(
                            validation(
                                x, y, dist, ker, not_fixed_window_size_calc, distance
                            )
                        )
...
encoded_categorical_columns = pd.DataFrame(
    onehot_encoder.fit_transform(dataset[categorical_columns])
)
